...
class VESDE(SDE):
  ...
  def discretize(self, x, t):
    ...
    timestep = (t * (self.N - 1) / self.T).long().cpu()
    sigma = self.discrete_sigmas[timestep].to(t.device)
    adjacent_sigma = torch.where(timestep.to(t.device) == 0, torch.zeros_like(t),
